...
def removeLegs(trip) -> int:
    success = 0
    if not trip.empty:
        for index, row in trip.iterrows():
            sl_no = row['Sl No.']
            # Remove from df
            df.drop(df[df['Sl No.'] == sl_no].index, axis=0, inplace=True)
            # Remove from start_legs if Departure Place is START_PLACE
            if row['Departure Place'] == START_PLACE:
                start_legs.drop(start_legs[start_legs['Sl No.'] == sl_no].index, axis=0, inplace=True)
                success += 1
...
## MAIN PART
def main(preprocessed_df):
    global df, start_legs
    # Initializations
    # df = pd.read_excel(preprocessed_df).reset_index(drop=True) 
    df = preprocessed_df.reset_index(drop=True)
...
#         self.START_PLACE = 'PSL'
#         self.MIN_DUTY = 7 * 3600  # 7 hours + 15 min sign in and 15 min sign out
#         self.MAX_DUTY = 9 * 3600  # 9 hours + 15 min sign in and 15 min sign out
#         self.MAX_SPREAD_OVER = 43200  # 12 hours
#         self.MIN_TERMINAL_GAP = 300  # 5 min
#         self.MAX_TERMINAL_GAP = 900  # 15 min
#         self.BREAK = 1800  # 30 min break to avoid 5 hours continuous working
#         self.IS_BREAK = False
#         self.MAX_SPLIT = 3600  # 1 hour split time
#         self.MIN_TEMPSET_SIZE = 2  # Minimum number of legs in a temp_set
#         self.BREAK_LIMIT_DUTY = 18000  # 5 hours of continuous duty
#         self.MIN_LIMIT_DUTY = 12600  # After 3.5 hours of duty, try entering the 30 min break
#         self.success = 0
#         self.trips_count = 0
#         self.exe_generateTempSet = 0
#         self.exe_popTempSet = 0
...
#     def generateTempSet(self, leg, mode=0):
#         # Function to generate temp_set for a particular leg in the stack
#         # mode 0 means normal trips and mode 1 means consider 30 min break also
#         # FN LOGIC IS NOT CORRECT , NEED TO CREATE THE TEMP SET ONLY WITH THE CRITERIA
#         # SORTING THEM AFTER CREATING THE ENTIRE TEMPSET IS NOT OPTIMAL
#         global exe_generateTempSet
...
#         # Creating a copy of the main legs.Can be limited with steering duty remaining 
#         temp_set_df = self.df[(self.df['Departure Place'] == leg['Arrival Place'])
#                         & (self.df['Departure Time'] > leg['Arrival Time'])]
#         temp_set_df = temp_set_df.sort_values(
#             'Departure Time', ascending=True).reset_index(drop=True)
#         temp_set_df['Terminal Gap'] = temp_set_df['Departure Time'] - leg['Arrival Time']
#         # Sorting and resetting index
...
#         temp_set.loc[:, 'Duty'] = leg['Duty'] + temp_set['Running Time'] # should bee run for all legs
#         # temp_set.loc[temp_set['Terminal Gap'] < MAX_TERMINAL_GAP, 'Duty'] += temp_set.loc[temp_set['Terminal Gap'] < MAX_TERMINAL_GAP, 'Terminal Gap']
#         mask = temp_set['Terminal Gap'] < self.MAX_TERMINAL_GAP
#         temp_set.loc[mask, 'Duty'] += temp_set.loc[mask, 'Terminal Gap']
#         y = time.time()
#         exe_generateTempSet += (y - x)
#         # if len(temp_set) < 2 : print(f"Size of tempset == {len(temp_set)}")
...
#         spread_over = self.secToTime((stack[-1]['current_leg'])['Arrival Time'] - (stack[0]['current_leg'])['Departure Time'])
#         frame = frame.drop('Duty', axis=1)
#         trips_count += 1
#         return [frame, duty, spread_over]
...
# #Remove Legs
# # - Once a leg is taken for a trip , it should be removed so that it will not be considered for the next round of iterations  
#     def removeLegs(self,trip) -> None:
#         global success
#         for i in range(len(trip)):
#             if trip.iloc[i]['Departure Place'] == self.START_PLACE:
#                 # remove from both df and start_legs 
#                 self.df.drop((self.df[self.df['Sl No.'] == trip.iloc[i]['Sl No.']]).index, axis= 0, inplace=True)
#                 self.start_legs.drop((self.start_legs[self.start_legs['Sl No.'] == trip.iloc[i]['Sl No.']]).index, axis= 0, inplace=True)
...
#                 self.backtrack(stack)
#             else:
#                 if (top_leg:=stack[-1]['current_leg'])['Arrival Place']==self.START_PLACE and top_leg['Duty'] > self.MIN_DUTY \
#                 and stack[-1]["break"]==True: 
#                     # break
#                     return stack
...
#     def main(self, df):
#         self.df = df
#         self.df['Terminal Gap'] = [0] * len(self.df)
#         self.df['Duty'] = [0] * len(self.df)
#         self.start_legs = self.df[self.df['Departure Place'] == self.START_PLACE].sort_values('Departure Time', ascending=True)
#         self.start_legs.reset_index(drop=True, inplace=True)
#         self.start_legs['Duty'] = self.start_legs['Running Time']  # Initial running time of start legs as its running time itself
#         trips_json = []  # List to store JSON representations of each trip
#         trip_number = 1  # Initialize trip number
...
#         # Construct the return value containing trip details
#         return json.dumps({
#             "success_legs": self.success,
#             "missing_routes": 104 - self.success,
#             "number_of_trips": self.trips_count,
#             "trips_details": trips_json
#         }, indent=4)
